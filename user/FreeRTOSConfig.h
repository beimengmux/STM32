...
#define configUSE_TICKLESS_IDLE 0                      //置 1：使能低功耗 tickless 模式；置 0：持系统节拍（tick）中断一直运行
...
#define configCHECK_FOR_STACK_OVERFLOW 1               // 大于 0 时启用堆栈溢出检测功能，
//如果使用此功能用户必须提供一个栈溢出钩子函数如果使用的话此值可以为 1 或者 2，因
//为有两种栈溢出检测方法
...
#define configMAX_PRIORITIES ( 8 )                              //可使用的最大优先级
