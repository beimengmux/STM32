...
#define PI 3.14159
#define Delay_us Delay_nopus
#define Delay_ms Delay_nopms
...
void LCD_FSMCConfig(void)
{
  ...
  readWriteTiming.FSMC_DataSetupTime = 0x0f;		 //数据保存时间为16个HCLK,因为液晶驱动IC的读数据的时候，速度不能太快C。
...
//		else if(sizey==32)temp=ascii_3216[num][i];		 //调用16x32字体
		...
		for(t=0;t<8;t++)
		{
			...
			{
				if(temp&(0x01<<t))LCD_Fast_DrawPoint(x,y,fc);//画一个点
				x++;
				if((x-x0)==sizex)
				{
					x=x0;
...
/******************************************************************************
函数说明：设置起始和结束地址
入口数据：xsc,xec 设置列的起始和结束地址
					ysp,yep 设置行的起始和结束地址
返回值：  无
******************************************************************************/
void LCD_Address_Set(u16 xsc,u16 ysp,u16 xec,u16 yep)
{
	//发送设置列地址命令
	LCD->LCD_REG = 0x2A;//列地址设置
	//发送起始列地址
	LCD->LCD_RAM = (xsc>>8)&0xff;
	LCD->LCD_RAM = (xsc)&0xff;
	//发送终点列地址
	LCD->LCD_RAM = (xec>>8)&0xff;
	LCD->LCD_RAM = (xec)&0xff;
	//发送设置页地址命令
	LCD->LCD_REG = 0x2B;//行地址设置
	//发送起始页地址
	LCD->LCD_RAM = (ysp>>8)&0xff;
	LCD->LCD_RAM = (ysp)&0xff;
	//发送终点页地址
	LCD->LCD_RAM = (yep>>8)&0xff;
	LCD->LCD_RAM = (yep)&0xff;
	//发送写显存命令
	LCD->LCD_REG = 0x2C;//写显存
}
/*
显示任意大小字体的函数
x,y -- 要显示的起始位置
...
data - 指向字模
*/
void LCD_ShowxXx(u16 x,u16 y,u16 w,u16 h,u16 fc,u16 bc,u8 *data)
{
	int i=0,j=0,k=0;
	//1.设置字模显示的区域
	LCD_Address_Set(x,y,x+w-1,y+h-1);//设置显示范围
	//2.给每个像素点先颜色值 -- 阴码 1=fc 0=bc
	for(j=0;j<h;j++)//高度 -- 多少行
	{
		for(i=0;i<w/8;i++)//宽度 -- 多少列 -- 一行几个字节
		{
			for(k=0;k<8;k++)//每个字节 -- 判断每个位
			{
				if((*data & (1<<k)) == 0)//背景色
					LCD->LCD_RAM = bc;
				else//不为0，就为前景色
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_Circle(u16 x0,u16 y0,u8 r,u16 fc)
{
	int a,b;
	a=0;b=r;	  
	while(a<=b)
	{
		LCD_Fast_DrawPoint(x0-b,y0-a,LCD->LCD_RAM = fc);             //3           
		LCD_Fast_DrawPoint(x0+b,y0-a,LCD->LCD_RAM = fc);             //0           
		LCD_Fast_DrawPoint(x0-a,y0+b,LCD->LCD_RAM = fc);             //1                
		LCD_Fast_DrawPoint(x0-a,y0-b,LCD->LCD_RAM = fc);             //2             
		LCD_Fast_DrawPoint(x0+b,y0+a,LCD->LCD_RAM = fc);             //4               
		LCD_Fast_DrawPoint(x0+a,y0-b,LCD->LCD_RAM = fc);             //5
		LCD_Fast_DrawPoint(x0+a,y0+b,LCD->LCD_RAM = fc);             //6 
		LCD_Fast_DrawPoint(x0-b,y0+a,LCD->LCD_RAM = fc);             //7
		a++;
...
{
	LCD_Line(x0,y0-r0,x0+r0*cos(54/(2*PI)),y0+r0*sin(54/(2*PI)),color);
	LCD_Line(x0+r0*cos(54/(2*PI)),y0+r0*sin(54/(2*PI)),x0-r0*cos(18/(2*PI)),y0-r0*sin(18/(2*PI)),color);	
	LCD_Line(x0-r0*cos(18/(2*PI)),y0-r0*sin(18/(2*PI)),x0+r0*cos(18/(2*PI)),y0-r0*sin(18/(2*PI)),color);
	LCD_Line(x0+r0*cos(18/(2*PI)),y0-r0*sin(18/(2*PI)),x0-r0*cos(54/(2*PI)),y0+r0*sin(54/(2*PI)),color);
	LCD_Line(x0-r0*cos(54/(2*PI)),y0+r0*sin(54/(2*PI)),x0,y0-r0,color);
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					...
					{
						if(tfont12[k].Msk[i]&(0x01<<j))	LCD_Fast_DrawPoint(x,y,fc);//画一个点
						x++;
						if((x-x0)==sizey)
						{
							x=x0;
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					...
					{
						if(tfont16[k].Msk[i]&(0x01<<j))	LCD_Fast_DrawPoint(x,y,fc);//画一个点
						x++;
						if((x-x0)==sizey)
						{
							x=x0;
