...
void Flash_Test(void)
{
	...
	printf("%#x\r\n",id);
	//2.给第7块第0扇区第0页写个字符串
	sFLASH_EraseSector(0x0800FD);                           //擦除
	//sFLASH_WritePage((u8 *)"CHINA",0x070000,6);
	//sFLASH_WritePage((u8 *)"CHINA",0x0800FD,6);
	sFLASH_WriteBuffer((u8 *)"CHINA",0x0800FD,6);           //写入
	//3.读取出来，在串口显示一下
	//sFLASH_ReadBuffer(buff,0x070000,6);
	sFLASH_ReadBuffer(buff,0x0800FD,6);                     //读取
	printf("%s\r\n",buff);
}
/**
  * @}
