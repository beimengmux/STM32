...
static void disp_init(void)  //接口的适配
{
    /*You code here*/
	LCD_Init ();        //屏幕初始化
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
	...
	//接口的适配10 屏幕的填充函数   lvgl知道某个区域要显示什么颜色，会调用disp_flush，我们要把显示屏的填充函数放到LVGL的填充函数接口中，
	LCD_Color_Fill(area->x1,area->y1,area->x2,area->y2,(uint16_t *)color_p);//这样lvgl调用disp_flush函数时，就会调用LCD_Color_Fill，然后显示在显示屏上
